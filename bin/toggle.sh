...
case "$TARGET" in
  ...
  tune)
    case "$TUNE_KEY" in
      ...
      # anything unusual).
      ONDEVICE_MAX_CHARS|TTS_CHUNK_CHARS|TTS_CHUNK_RETRIES|TTS_RETRY_WAIT_BASE|TTS_RETRY_WAIT|PREFLIGHT_TIMEOUT|WARM_SKIP_WINDOW|OVERFLOW_OPENING_CHARS|WATCH_INTERVAL|LOG_MAX_BYTES|TTS_RATE|TTS_PITCH|NOTIFY_COOLDOWN|ELEVENLABS_GAIN|ELEVENLABS_MODEL|ELEVENLABS_SPEED|GEMINI_MODEL|GEMINI_SPEED|INWORLD_MODEL|INWORLD_SPEAKING_RATE|CLOUD_HTTP_TIMEOUT|CLOUD_CHUNK_CHARS|CLOUD_FIRST_CHUNK_CHARS|CLOUD_PLAY_LEAD|STARTUP_GREETING_TEXT) ;;
      *)
        echo "unknown tuning key: $TUNE_KEY" >&2
        echo "valid keys: ONDEVICE_MAX_CHARS TTS_CHUNK_CHARS TTS_CHUNK_RETRIES TTS_RETRY_WAIT_BASE TTS_RETRY_WAIT PREFLIGHT_TIMEOUT WARM_SKIP_WINDOW OVERFLOW_OPENING_CHARS WATCH_INTERVAL LOG_MAX_BYTES TTS_RATE TTS_PITCH NOTIFY_COOLDOWN ELEVENLABS_GAIN ELEVENLABS_MODEL ELEVENLABS_SPEED GEMINI_MODEL GEMINI_SPEED INWORLD_MODEL INWORLD_SPEAKING_RATE CLOUD_HTTP_TIMEOUT CLOUD_CHUNK_CHARS CLOUD_FIRST_CHUNK_CHARS CLOUD_PLAY_LEAD STARTUP_GREETING_TEXT" >&2
        exit 1
        ;;
    esac
