...
speak_cloud_chunked() {
  local backend="$1" text="$2" cap="$3" seed="${4:-}" plan="${5:-}" playing="${6:-}" seed1="${7:-}"
  ...
  # SEED1 (7th arg): chunk 1, pre-generated by the same speculative generator
  # that produced the seed. It is usually still in flight when we get here —
  # chunk 0 finished first, which is what let the handover happen — so this
  # adopts the generation already running instead of starting a second one. The
  # adopter stands in for a generator: the loop waits on this PID and reads its
  # exit status exactly as it would a gen_cloud, and if the pre-generation
  # failed or never lands it generates the chunk the ordinary way rather than
  # dropping the readout to on-device.
  if [ -n "$seed1" ] && [ "$n" -gt 1 ] && [ -z "${gen_done[1]:-}" ]; then
    (
      w=0
      while [ ! -s "${seed1}.rc" ] && [ "$w" -lt 120 ]; do sleep 0.5; w=$(( w + 1 )); done
      if [ "$(cat "${seed1}.rc" 2>/dev/null)" = "0" ] && [ -s "$seed1" ] \
         && mv -f "$seed1" "$(_cloud_audio_path "$backend" 1)" 2>/dev/null; then
        rm -f "${seed1}.rc" 2>/dev/null
      else
        rm -f "$seed1" "${seed1}.rc" 2>/dev/null
        gen_cloud "$backend" "${chunks[1]}" 1 || exit 1
      fi
      printf '%s' "${EPOCHREALTIME:-$(date +%s.%N)}" > "$(_cloud_audio_path "$backend" 1).gen"
    ) &
    gen_pid[1]=$!
  fi
...
_hyb_spec_launch() {
  ...
  (
    ...
    IFS= read -r -d '' second <&3 2>/dev/null || true
    exec 3<&- 2>/dev/null || true
    [ -n "$first" ] || first="$(_cloud_first_chunk "$suffix_text")"
    # Chunk 1 is generated here too, alongside chunk 0 rather than after it.
    # Leaving it to speak_cloud_chunked meant it only started at the handover,
    # by which time chunk 0 was already sounding — so the ONE thing covering its
    # generation was chunk 0's own playback, ~9s of it, against ~9.5s of
    # generation. Measured 2026-07-28: slack -2.7s, i.e. 2.8s of silence right
    # after the voices changed. In the non-hybrid path chunk 1 gets chunk 0's
    # generation time as a head start as well; pre-generating restores that, and
    # the on-device opening (9-17s) is a bigger head start than either.
    # Parallel, not sequential: two generations in flight do not slow each other
    # (measured, 2- and 3-way), and chunk 0 must not be made to wait — the
    # handover is gated on its .rc alone, which is written the moment it lands.
    if [ -n "$second" ]; then
      ( gen_cloud "$backend" "$second" "hyb-$b-1"
        printf '%s' "$?" > "$(_cloud_audio_path "$backend" "hyb-$b-1").rc" ) &
    fi
...
_hyb_spec_discard() {
  ...
  # The candidate's chunk 1, generated alongside it. A discarded candidate's
  # chunk 1 is audio of text the listener is about to hear read on-device
  # instead, so leaving it behind would be worse than wasteful.
  local out1; out1="$(_cloud_audio_path "$1" "hyb-$2-1")" || return 0
  rm -f "$out1" "${out1}.rc" "${out1%.wav}.pcm" "${out1%.mp3}-raw.mp3" 2>/dev/null
  return 0
}
      ...
      case "$state" in
        ok)
          ...
          if speak_cloud_chunked "$backend" "${suffix[$b]}" "$cap" "$seed" "${seed}.plan" \
               "$_HYB_PREPLAY_STARTED" "$(_cloud_audio_path "$backend" "hyb-$b-1")"; then
            VOICE_READOUT_KEEP_WAKELOCK=""; ondevice_wake_unlock_held
            return 0
          fi
