...
| 「ハイブリッドをオンにして」 | **フル ハイブリッド**：フル読み上げ＋クラウド音声のとき、出だしだけオンデバイスで即座に喋り出し、途中でクラウド音声に交代する（裏で `toggle.sh hybrid on`。既定オフ。下の「フル ハイブリッド」参照） |
| 「ハイブリッドをオフにして」 | 通常のクラウド読み上げに戻す（デフォルト） |
...
| `TTS_CHUNK_CHARS` | 150 | オンデバイス読み上げのチャンク長（1回の発話ごとに約2.7秒の立ち上がりがかかるので、短くするほど文の途中の無音が増える） |
...
| `HYBRID_UNIT_CHARS` | 60 | ハイブリッドのオンデバイス側の1単位＝**交代の粒度**。クラウドが間に合ったかを単位の境目ごとに判定する |
| `HYBRID_MAX_ONDEVICE_CHARS` | 240 | オンデバイスで読んだ累計がこれを超えたら、読み進めるのをやめてクラウドを待つ側に倒す |
| `HYBRID_SPECULATION` | 1 | 交代候補を同時にいくつ先行生成するか。2 にすると毎回1本ぶん捨てる代わりに、クラウドが遅いときの復帰が1単位ぶん速くなる |
...
クラウド音声は良いが、最初の音が出るまでが遅い。生成に数秒、その音を鳴らす `termux-media-player` の往復にさらに約2秒かかる。一方オンデバイスのエンジンは約3秒で喋り出し、ネットワークも要らない。**フル ハイブリッド**はその差を使う。出だしをオンデバイスで読んでいる間にクラウド音声を作り、間に合った時点で声を交代する。
...
- **交代の段取りも先に済ませてある。** 音声が出来ていても、チャンク分割・一時ファイルの掃除・ウェイクロックの解放は数秒かかり、そのまま置くと全部が「オンデバイスが黙ってからクラウドが鳴るまで」の無音に乗る（実測7秒）。分割と掃除は先行生成と同じ裏側の処理でユニットを読んでいる間に終わらせ、ウェイクロックは読み上げ1本を通して持ったまま最後にまとめて解放する。継ぎ目に残るのは `termux-media-player` の再生往復ぶんだけになる
