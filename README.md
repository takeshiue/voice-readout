...
上限を超えたときの挙動は**機能によって意図的に非対称**にしている：

| 機能 | オンデバイスで上限超過かつクラウドキー無し |
|---|---|
| フル読み上げ（Stop・full） | **一文要約に劣化**して読む（無音よりはマシ、を優先）。要約自体が240字を超える場合は240字に切り詰める |
| ファイル読み上げ（`speak-text.sh`） | **読み上げず中止**し、クラウド系バックエンドへの切替を案内する |

フルとファイルで扱いが違うのは「機能ありき」の考え方による。フル読み上げは「応答を追えるようにする」機能なので、多少削ってでも声を出す方が目的に適う。一方ファイル読み上げは「このテキストを読んで」と明示的に渡された機能で、勝手に要約するのは頼まれたことと別物になる——だから中止して切替を促す。

クラウド系バックエンド（Gemini／Inworld／ElevenLabs）にはこの上限は無い。長文を丸ごと読ませたい場合は、その機能のエンジンをクラウド系に切り替える（例：`toggle.sh backend-file elevenlabs`）。
