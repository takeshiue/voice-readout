#!/bin/bash
# Test Progressive Ramp-up Pipeline (Measuring exact generation times & margins for Chunk 0, 1, 2)
set -u
...
TMP_TRANSCRIPT="/tmp/test_agy_transcript_progressive.jsonl"
...
echo "Launching Progressive Ramp-up Test (Watching Chunk 0 Generation Time & Margins closely)..."
printf '{"transcriptPath": "%s"}' "$TMP_TRANSCRIPT" > /tmp/meas_progressive_input.json
"$ROOT_DIR/bin/agy-summarize-and-speak.sh" __worker /tmp/meas_progressive_input.json
